...
{
    public partial class VariableTitleBox
    {
        public override RenderingResult Validate(IReportingDataProvider dataProvider, IRenderingState renderingState, ITree<ReportElementId> elementTree)
        {
            ...
            {
                var setterElement = "VariableTitleContainer";
                var message = "The Variable Title Range's Repeat Group should have already been set by the " + setterElement + ".";
                throw new InvalidOperationException(message);
            }

            if (!m_ContainedVariableTitleRangeNumber.HasValue)
            { throw new InvalidOperationException("The contained VariableTitleRange has not been set."); }
            if (!renderingState.ReportElements.ContainsKey(new ReportElementId(this.ParentReportId, m_ContainedVariableTitleRangeNumber.Value)))
            { throw new InvalidOperationException("The contained VariableTitleRange does not exist in the current Report."); }
