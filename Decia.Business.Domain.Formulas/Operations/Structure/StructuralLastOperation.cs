...
{
    public class StructuralLastOperation : OperationBase
    {
        public const bool Default_UseOrdering = StructuralFirstOperation.Default_UseOrdering;
        public const int Default_SkipCount = StructuralFirstOperation.Default_SkipCount;
        ...
        public StructuralLastOperation()
        {
            m_Id = OperationCatalog.GetOperationId(this.GetType());
            m_ShortName = "LAST";
            m_LongName = "Last";
            m_Description = "Aggregates across Structural Dimension(s) by selecting the last available value.";
        ...
        protected override OperationMember DoCompute(IFormulaDataProvider dataProvider, ICurrentState currentState, IFormula parentFormula, IExpression callingExpression, IDictionary<int, OperationMember> inputs, OperationMember defaultReturnValue)
        {
                ...
                || SignatureSpecification.IsSpecificInstanceOf(Input_Text_Bool_Int__Return_Text, signature))
            {
                ...
                { result = aggregationValuesList.Reverse<ChronometricValue>().FirstOrDefault(); }
                else if (!useOrdering && (skipCount > 0))
                { result = aggregationValuesList.Reverse<ChronometricValue>().Skip(skipCount).FirstOrDefault(); }
                else if (useOrdering && (skipCount <= 0))
                { result = aggregationValuesList.OrderBy(x => x.GetValue(currentState.TimeKey).GetValue().ToString()).Reverse().FirstOrDefault(); }
                else
                { result = aggregationValuesList.OrderBy(x => x.GetValue(currentState.TimeKey).GetValue().ToString()).Reverse().Skip(skipCount).FirstOrDefault(); }
        ...
        protected override string DoRenderAsSql(IFormulaDataProvider dataProvider, ICurrentState currentState, SqlFormulaInfo exportInfo, IFormula parentFormula, IExpression callingExpression, IDictionary<int, string> argumentTextByIndex)
        {
            ...
            exportInfo.RowLimiting_OrderByText = string.Format("ORDER BY {0} DESC", exportInfo.Filtering_ResultColumnText);
