...
{
    public class CurrentPeriodStartDateOperation : OperationBase
    {
        ...
        public CurrentPeriodStartDateOperation()
        {
            m_Id = OperationCatalog.GetOperationId(this.GetType());
            m_ShortName = "TPER_START_DATE";
            m_LongName = "Current Period Start Date";
            m_Description = "Gets the Start Date of the current computation Period for a given Time Dimension number.";
                ...
                || SignatureSpecification.IsSpecificInstanceOf(Input_Dubl_Text__Return_Date, signature))
            {
                ...
                foreach (var primaryTimePeriod_In in primaryTimePeriods_In)
                {
                    foreach (var secondaryTimePeriod_In in secondaryTimePeriods_In)
                    {
                        ...
                        var timePeriodStartDate_Out = DeciaDataTypeUtils.MinDateTime;

                        if (!timePeriod_ToShow.IsForever)
                        { timePeriodStartDate_Out = timePeriod_ToShow.StartDate; }

                        var timeKey_Out = new MultiTimePeriodKey(primaryTimePeriod_In, secondaryTimePeriod_In);
                        timeKeys_Out.Add(timeKey_Out);

                        result.SetValue(timeKey_Out, timePeriodStartDate_Out);
                    }
                }
        ...
        protected override string DoRenderAsSql(IFormulaDataProvider dataProvider, ICurrentState currentState, SqlFormulaInfo exportInfo, IFormula parentFormula, IExpression callingExpression, IDictionary<int, string> argumentTextByIndex)
        {
            ...
            if (isForever)
            {
                var startDateAsString = DeciaDataType.DateTime.ToStringValue(TimePeriodTypeUtils.SqlServerMinDate_SmallDateTime);
                result = string.Format("CONVERT(datetime, '{0}', 126)", startDateAsString.Substring(0, startDateAsString.Length - 4));
            }
            else
            {
                result = string.Format("{0}.[{1}]", timePeriod_TableAlias, DeciaBaseUtils.Included_TimePeriod_StartDate_ColumnName);
            }
            return result;
        }
    }
